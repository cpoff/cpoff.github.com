...
// Otherwise the form was just submitted, so process form.
    ...
    // Otherwise the user selected to save the profile, so save it.
        ...
        if (
            (
                ($liveform->get_field_value('action') == 'order_abandoned')
                || ($liveform->get_field_value('action') == 'order_completed')
                || ($liveform->get_field_value('action') == 'product_ordered')
            )
            &&
            (
                (ECOMMERCE == false)
            ...
            $liveform->assign_field_value('action', '');
        }
