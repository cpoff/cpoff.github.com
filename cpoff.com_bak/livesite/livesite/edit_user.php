...
if (!$_POST) {
    ...
    print
        ...
        <div id="button_bar">
            ' . $output_login_as_user_button . '
            <a href="#" onclick="if (confirm(\'Are you sure you want to reset the user\\\'s password and email a new, temporary password to the user?\') == true) {document.reset_password_form.submit(); return false;} else {return false;}">Reset &amp; Send Password</a><form name="reset_password_form" action="reset_password.php" method="post">' . get_token_field() . '<input type="hidden" name="send_to" value="' . h($_REQUEST['send_to']) . '" /><input type="hidden" name="user_id" value="' . h($_GET['id']) . '"></form>
        </div>
        <div id="content">
            
            <a href="#" id="help_link">Help</a>
            <h1>Edit User</h1>
            <div class="subheading">Update this user\'s privileges, or email a new temporary password.</div>
