...
function get_express_order($properties) {
    ...
    $shipping_address_and_arrival_page_id = $properties['shipping_address_and_arrival_page_id'];
    ...
    $_SESSION['ecommerce']['shipping_address_and_arrival_page_id'] = $shipping_address_and_arrival_page_id;
    
    $form = new liveform('express_order');
    
    // if a reference code was passed in the query string, retrieve order for user
    if ($_GET['r'] || $_GET['reference_code']) {
        ...
        $query = "SELECT id, status FROM orders WHERE reference_code = '" . escape($reference_code) . "'";
        $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
        
        // if an order was found for the reference code
        if (mysqli_num_rows($result) > 0) {
            $row = mysqli_fetch_assoc($result);
            $order_id = $row['id'];
            $status = $row['status'];
            
            // if order is not complete, store order id in session
            if ($status == 'incomplete') {
                ...
                if (VISITOR_TRACKING == true) {
                    $query = "UPDATE visitors
                             ...
                             WHERE (id = '" . $_SESSION['software']['visitor_id'] . "') AND (order_created = '0') AND (order_retrieved = '0')";
                    $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
                }
                
            // else order is complete, so output error
            } else {
                output_error('The ' . h($shopping_cart_label) . ' has already been completed. You may only retrieve incomplete ' . h(mb_strtolower($shopping_cart_label)) . 's.');
            }
            
        // else an order was not found for the reference code, so output error
        } else {
            output_error('The ' . h($shopping_cart_label) . ' (' . h($reference_code) . ') could not be found.');
        }
    }
        ...
        if ($ship_tos) {
            ...
            foreach ($ship_tos as $ship_to_id) {
                ...
                foreach ($order_items as $order_item) {
                    ...
                    $output_remove = '<a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/remove_item_from_cart.php?order_item_id=' . $order_item_id . '&screen=express_order&send_to=' . h(urlencode(get_request_uri())) . get_token_query_string_field() . '" class="software_button_small_secondary remove_button">X</a>';
                ...
                if ($output_products) {
                    ...
                    if ((ECOMMERCE_SHIPPING == true) && ($ship_to_id != 0)) {
                        ...
                        $output_ship_to_incomplete_message = '';
                        
                        // if ship to is incomplete, then prepare message
                        if ($ship_to_complete == 0) {
                            $output_ship_to_incomplete_message = '<span class="software_highlight">(Incomplete)</span> &nbsp;';
                        }
                        ...
                        if ($address) {
                            $output_shipping_address = $address;
                        }
                        
                        $output_arrival_date = '';
                        
                        // if arrival dates exist
                        if ($arrival_dates_exist == true) {
                            // get arrival date information
                            $query = "SELECT name, custom FROM arrival_dates WHERE id = '$arrival_date_id'";
                            $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
                            
                            // if there is an arrival date selected for this ship to, then prepare requested arrival date content
                            if (mysqli_num_rows($result) > 0) {
                                $row = mysqli_fetch_assoc($result);
                                ...
                                $output_arrival_date = '<div>Requested Arrival Date: ' . $output_arrival_date . '</div>';
                            }
                        }
                        ...
                        $output_ship_tos .=
                            '<tr class="ship_tos">
                                <td colspan="6">
                                    <div class="heading">Ship to ' . $output_ship_to_name . $output_ship_to_incomplete_message . '<a class="software_button_tiny_secondary" href="' . OUTPUT_PATH . get_page_name($_SESSION['ecommerce']['shipping_address_and_arrival_page_id']) . '?ship_to_id=' . $ship_to_id . '">Update</a></div>
                                    <div class="data">
                                        ' . $output_shipping_address . '
                    ...
                    // if shipping is on and this is a real ship to and ship to is complete, output shipping row
                    if ((ECOMMERCE_SHIPPING == true) && ($ship_to_id != 0) && ($ship_to_complete == 1)) {
                        ...
                        // if there is a shipping method name or shipping method description, then the shipping method has been selected, so prepare to output details
                        if ($shipping_method_name || $shipping_method_description) {
                            $shipping_description = $shipping_method_name;
                            
                            if ($shipping_method_description) {
                        ...
                        // assume that the shipping is not discounted, until we find out otherwise
                        ...
                        if ($ship_to_offer_id != 0) {
                            $original_price = $original_shipping_cost;
                            $discounted = TRUE;
                        ...
                        $output_shipping =
                            '<tr class="ship_tos data">
                                <td class="mobile_left" colspan="2" style="vertical-align: top">Shipping Method: ' . h($shipping_description) . '</td>
                                <td class="mobile_hide">&nbsp;</td>
                                <td class="mobile_hide">&nbsp;</td>
                                <td class="mobile_right"style="vertical-align: top; text-align: right">' . prepare_price_for_output($original_price * 100, $discounted, $discounted_price * 100, 'html') . '</td>
                                <td class="mobile_hide">&nbsp;</td>
                            </tr>';
                    }
            ...
            if ($special_offer_code_label != '' || $special_offer_code_message != '' || $special_offer_code != '') {
                $output_special_offer =
                    '<table style="margin-bottom: 15px">
                        <tr>
                            <td class="mobile_left">' . h($special_offer_code_label) . '</td>
                            <td class="mobile_hide" style="padding-right: .5em;"></td>
                            <td class="mobile_left mobile_width">' . $form->output_field(array('type'=>'text', 'name'=>'special_offer_code', 'size'=>'15', 'maxlength'=>'50', 'class'=>'software_input_text')) . '</td>
                            <td class="mobile_hide" style="padding-right: .5em;"></td>
                            <td class="mobile_left mobile_width">' . h($special_offer_code_message) . '</td>
                        </tr>
                    </table>';
            } else {
                $output_special_offer = '';
            ...
            // if user is logged in, get contact for user (we will use this contact id in a couple of places)
            ...
            if ($form->field_in_session('billing_first_name') == false) {
                ...
                if (USER_LOGGED_IN && ($_SESSION['software']['update_contact'] === false)) {
                ...
                // If a custom billing form is enabled then prefill those fields.
                if ($custom_billing_form == 1) {
                    $fields = db_items(
                        ...
                        LEFT JOIN form_fields ON form_data.form_field_id = form_fields.id
                        WHERE
                            (form_data.order_id = '" . $_SESSION['ecommerce']['order_id'] . "')
                    ...
                    foreach ($fields as $field) {
                        ...
                        $html_field_name = 'field_' . $field['form_field_id'];
            ...
            // get all states for javascript array
            $query = "SELECT states.name, states.code, countries.code as country_code
                     FROM states
            ...
            $index = 0;
            while ($row = mysqli_fetch_assoc($result)) {
                $output_states_array .= 'states[' . $index . '] = new Array("' . $row['name'] . '", "' . $row['code'] . '", "' . $row['country_code'] . '");' . "\n";
                $index++;
            }
            ...
            $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
            while ($row = mysqli_fetch_assoc($result)) {
                $billing_country_options[$row['name']] = $row['code'];
            }
            ...
            // If this visitor is logged in, then output check box to allow the visitor
            ...
            if ($custom_billing_form == 1) {
                ...
                $form_info = get_form_info($page_id, 0, 0, 0, $custom_billing_form_label_column_width, $office_use_only = false, $form, 'frontend', false, $device_type, $folder_id_for_default_value);
            ...
            if (
                ...
                // start the output for payment methods with javascript for showing and hiding fields
                $output_payment_methods =
                    '<script type="text/javascript">
                        // when the page has loaded, then show or hide payment method fields
                        software_$(document).ready(function () {
                            show_or_hide_payment_method_fields();
                        });
                        
                        function show_or_hide_payment_method_fields() {
                            // if the credit/debit card payment method exists, then show or hide fields for it
                            if (document.getElementById("payment_method_credit_debit_card")) {
                                if (document.getElementById("payment_method_credit_debit_card").checked == true) {
                                    document.getElementById("credit_debit_card_fields").style.display = "";

                                    // If a surcharge is enabled, then add it.
                                    if (software_$("#software_surcharge_row").length) {
                                        // Hide total row that does not include surcharge.
                                        document.getElementById("software_total_row").style.display = "none";

                                        // Show surcharge row and total row that includes surcharge.
                                        document.getElementById("software_surcharge_row").style.display = "";
                                        document.getElementById("software_surcharge_total_row").style.display = "";
                                    }

                                } else {
                                    document.getElementById("credit_debit_card_fields").style.display = "none";

                                    // If a surcharge is enabled, then remove it.
                                    if (software_$("#software_surcharge_row").length) {
                                        // Hide surcharge row and total row that includes surcharge.
                                        document.getElementById("software_surcharge_row").style.display = "none";
                                        document.getElementById("software_surcharge_total_row").style.display = "none";

                                        // Show total row that does not include surcharge.
                                        document.getElementById("software_total_row").style.display = "";
                                    }
                                }
                            }
                        }
                ...
                if ($show_credit_debit_card_payment_method == TRUE) {
                    ...
                    if ($card_verification_number_page_id) {
                        ...
                        $output_card_verification_number_help = ' <a href="' . OUTPUT_PATH . h(encode_url_path($card_verification_number_page_name)) . '" target="_blank">What is this?</a>';
                    }
                    ...
                    if (
                        ...
                        // Remember total with surcharge so if the total changes after the customer submits the order,
                        // we can forward the user back to review the new total.
                        $_SESSION['ecommerce']['total_with_surcharge'] = $grand_total_with_surcharge * 100;
                        ...
                        $output_surcharge_rows =
                            '<tr class="order_totals data" id="software_surcharge_row">
                            ...
                            <tr class="order_totals data" id="software_surcharge_total_row">
                                <td class="mobile_left" colspan="4" style="text-align: right"><strong>Total Due:</strong></td>
                                <td class="mobile_right" style="text-align: right"><strong>' . prepare_price_for_output($grand_total_with_surcharge * 100, FALSE, $discounted_price = '', 'html') . $output_unconverted_total . '</strong></td>
                            </tr>';
                    ...
                    $output_payment_methods .=
                        '<div class="payment data" style="margin-bottom: 1em">
                            <div>' . $form->output_field(array('type'=>'radio', 'name'=>'payment_method', 'id'=>'payment_method_credit_debit_card', 'value'=>'Credit/Debit Card', 'checked'=>$credit_debit_card_checked, 'class'=>'software_input_radio', 'required' => 'true', 'onclick'=>'show_or_hide_payment_method_fields(); document.getElementById(\'submit_purchase_now\').value = \'' . $standard_purchase_now_button_label . '\'')) . '<label for="payment_method_credit_debit_card"> Credit/Debit Card</label></div>
                            <div id="credit_debit_card_fields" style="padding-left: 3em; margin-top: .25em">
                                <table>
                                    <tr>
                                        <td>Card Type*</td>
                                        <td>' . $form->output_field(array('type'=>'select', 'name'=>'card_type', 'options'=>$card_type_options, 'class'=>'software_select')) . '</td>
                                    </tr>
                                    <tr>
                                        <td>Card Number*</td>
                                        <td>' . $form->output_field(array('type'=>'text', 'name'=>'card_number', 'size'=>'25', 'maxlength'=>'50', 'class'=>'software_input_text')) . '</td>
                                    </tr>
                                    <tr>
                                        <td>Expiration*</td>
                                        <td>' . $form->output_field(array('type'=>'select', 'name'=>'expiration_month', 'options'=>$expiration_month_options, 'class'=>'software_select')) . ' ' . $form->output_field(array('type'=>'select', 'name'=>'expiration_year', 'options'=>$expiration_year_options, 'class'=>'software_select')) . '</td>
                                    </tr>
                                    <tr>
                                        <td>Cardholder*</td>
                                        <td>' . $form->output_field(array('type'=>'text', 'name'=>'cardholder', 'maxlength'=>'50', 'class'=>'software_input_text')) . '</td>
                                    </tr>
                                    <tr>
                                        <td>Verification Number*</td>
                                        <td class="card_verification_number mobile_left">' . $form->output_field(array('type'=>'text', 'name'=>'card_verification_number', 'size'=>'4', 'maxlength'=>'4', 'class'=>'software_input_text')) . $output_card_verification_number_help . '</td>
                                    </tr>
                                </table>
                                ' . $output_surcharge_message . '
                            </div>
                ...
                if ($show_paypal_express_checkout_payment_method == TRUE) {
                    ...
                    $output_payment_methods .= '<div class="payment data" style="margin-bottom: 1em">' . $form->output_field(array('type'=>'radio', 'name'=>'payment_method', 'id'=>'payment_method_paypal_express_checkout', 'value'=>'PayPal Express Checkout', 'checked'=>$paypal_express_checkout_checked, 'class'=>'software_input_radio', 'required' => 'true', 'onclick'=>'document.getElementById(\'submit_purchase_now\').value = \'' . $paypal_express_checkout_purchase_now_button_label . '\'; show_or_hide_payment_method_fields()')) . '<label for="payment_method_paypal_express_checkout"> <img src="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/images/paypal.png" width="40" height="26" border="0" alt="PayPal" onclick="document.getElementById(\'payment_method_paypal_express_checkout\').checked = true; document.getElementById(\'submit_purchase_now\').value = \'' . $paypal_express_checkout_purchase_now_button_label . '\'; show_or_hide_payment_method_fields()" /></label></div>';
                }
                ...
                if (
                    ...
                    $output_payment_methods .= '<div class="payment data" style="margin-bottom: 1em">' . $form->output_field(array('type'=>'radio', 'name'=>'payment_method', 'id'=>'payment_method_offline_payment', 'value'=>'Offline Payment', 'checked'=>$offline_payment_checked, 'class'=>'software_input_radio', 'required' => 'true', 'onclick'=>'show_or_hide_payment_method_fields(); document.getElementById(\'submit_purchase_now\').value = \'' . $standard_purchase_now_button_label . '\'')) . '<label for="payment_method_offline_payment"> ' . h($offline_payment_label) . '</label></div>';
                }
            ...
            // else the total is 0 or there is an active payment method, so prepare to output purchase now button
                ...
                // if the PayPal Express Checkout payment method will be the only outputted payment method or the PayPal Express Checkout payment method is selected,
                // then use PayPal Express Checkout purchase now button label
                if ((($show_paypal_express_checkout_payment_method == TRUE) && ($show_credit_debit_card_payment_method == FALSE)) || ($form->get_field_value('payment_method') == 'PayPal Express Checkout')) {
                    $output_purchase_now_button_label = $paypal_express_checkout_purchase_now_button_label;
                    
                // else PayPal Express Checkout payment method is not selected, so use standard purchase now button label
                } else {
                    $output_purchase_now_button_label = h($standard_purchase_now_button_label);
                }
                
                $output_purchase_now_button = '<input type="submit" name="submit_purchase_now" id="submit_purchase_now" value="' . $output_purchase_now_button_label . '" class="software_input_submit_primary purchase_button" style="margin: 0 0 .5em .5em"/>';
            }
            ...
            $output_express_order =
                ...
                <form action="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/express_order.php" method="post" style="margin: 0px 0px 15px 0px">
                    ...
                    <table class="products" style="width: 100%; margin-bottom: 15px">
                        ...
                        ' . $output_gift_card_discount . '
                        ' . $output_surcharge_rows . '
                        <tr class="ship_tos data" id="software_total_row">
                    ...
                    <div class="billing data">
                        <table style="margin-bottom: 1.5em">
                            ...
                            <tr>
                                <td>First Name*</td>
                                <td>' . $form->output_field(array('type'=>'text', 'name'=>'billing_first_name', 'maxlength'=>'30', 'class'=>'software_input_text', 'required' => 'true')) . '</td>
                            </tr>
                            <tr>
                                <td>Last Name*</td>
                                <td>' . $form->output_field(array('type'=>'text', 'name'=>'billing_last_name', 'maxlength'=>'30', 'class'=>'software_input_text', 'required' => 'true')) . '</td>
                            </tr>
                            <tr>
                                <td>Company</td>
                                <td>' . $form->output_field(array('type'=>'text', 'name'=>'billing_company', 'maxlength'=>'30', 'class'=>'software_input_text')) . '</td>
                            </tr>
                            <tr>
                                <td>Address 1*</td>
                                <td>' . $form->output_field(array('type'=>'text', 'name'=>'billing_address_1', 'maxlength'=>'30', 'class'=>'software_input_text', 'required' => 'true')) . '</td>
                            </tr>
                            <tr>
                                <td>Address 2</td>
                                <td>' . $form->output_field(array('type'=>'text', 'name'=>'billing_address_2', 'maxlength'=>'30', 'class'=>'software_input_text')) . '</td>
                            </tr>
                            <tr>
                                <td>City*</td>
                                <td>' . $form->output_field(array('type'=>'text', 'name'=>'billing_city', 'maxlength'=>'30', 'class'=>'software_input_text', 'required' => 'true')) . '</td>
                            </tr>
                            <tr>
                                <td><div class="state_label">State / Province</div></td>
                                <td><div id="billing_state_container">' . $form->output_field(array('type'=>'text', 'name'=>'billing_state', 'id'=>'billing_state', 'maxlength'=>'50', 'class'=>'software_input_text')) . '</div></td>
                            </tr>
                            <tr>
                                <td>Zip / Postal Code*</td>
                                <td>' . $form->output_field(array('type'=>'text', 'name'=>'billing_zip_code', 'maxlength'=>'50', 'class'=>'software_input_text', 'required' => 'true')) . '</td>
                            </tr>
                            <tr>
                                <td>Country*</td>
                                <td>' . $form->output_field(array('type'=>'select', 'name'=>'billing_country', 'id'=>'billing_country', 'options'=>$billing_country_options, 'class'=>'software_select', 'required' => 'true', 'onchange'=>'change_states(\'billing_country\', \'billing_state\')')) . '<script type="text/javascript" language="JavaScript 1.2">change_states(\'billing_country\', \'billing_state\', true);</script></td>
                            </tr>
                            <tr>
                                <td>Phone*</td>
                                <td>' . $form->output_field(array('type'=>'tel', 'name'=>'billing_phone_number', 'maxlength'=>'20', 'class'=>'software_input_text', 'required' => 'true')) . '</td>
                            </tr>
                            <tr>
                                <td>Email*</td>
                                <td>' . $form->output_field(array('type'=>'email', 'name'=>'billing_email_address', 'maxlength'=>'50', 'class'=>'software_input_text', 'required' => 'true')) . '</td>
                            </tr>
                            ' . $output_opt_in . '
                            ' . $output_po_number . '
                            ' . $output_referral_source . '
                ...
                // remember total so if the total changes after the customer submits the order, we can forward the user back to review the new total
                $_SESSION['ecommerce']['total'] = $grand_total * 100;
    ...
    // Otherwise this is a custom layout.
        ...
        if ($recipients) {
            ...
            // if user is logged in, get contact for user
            if (USER_LOGGED_IN) {
                $contact = db_item(
                    "SELECT
                        id,
            ...
            if (!$form->field_in_session('billing_first_name')) {
                ...
                if ($custom_billing_form && $order_id) {
                    $fields = db_items(
                        "SELECT
                            form_data.form_field_id,
                            form_data.data,
            ...
            $form->set('billing_zip_code', 'required', true);
            ...
            // get countries for country selection
            $countries = db_items("SELECT name, code FROM countries ORDER BY name", 'code');
            ...
            // Loop through the countries in order to remove countries that don't have states,
            // We only need data for countries that have states, because that is all the JS cares about.
            ...
            if ($custom_billing_form) {
                ...
                if ($edit) {
                    ...
                    // If the form name is not blank, then add it to the title.
                    if ($custom_billing_form_name != '') {
                        $title = ': ' . h($custom_billing_form_name);
                    } else {
                        $title = '';
                    }
                    ...
                    $edit_custom_billing_form_end = '</div>';
            ...
            if (
                ...
                if ((ECOMMERCE_CREDIT_DEBIT_CARD == true) && ((ECOMMERCE_AMERICAN_EXPRESS === true) || (ECOMMERCE_DINERS_CLUB == true) || (ECOMMERCE_DISCOVER_CARD == true) || (ECOMMERCE_MASTERCARD == true) || (ECOMMERCE_VISA == true))) {
                    ...
                    // If credit/debit card is the only payment method,
                    // then prepare fields.
                    if (
                        ...
                        $form->set('card_type', 'required', true);
                        $form->set('card_number', 'required', true);
                        $form->set('expiration_month', 'required', true);
                        $form->set('expiration_year', 'required', true);
                        $form->set('cardholder', 'required', true);
                        $form->set('card_verification_number', 'required', true);
                    ...
                    $form->set('card_number', 'maxlength', 50);
                    ...
                    $form->set('cardholder', 'maxlength', 50);
                    ...
                    if (
                        ...
                        // Remember total with surcharge so if the total changes after the customer submits the order,
                        // we can forward the user back to review the new total.
                        $_SESSION['ecommerce']['total_with_surcharge'] = $total_with_surcharge * 100;
            ...
            // remember total so if the total changes after the customer submits the order, we can forward the user back to review the new total
            $_SESSION['ecommerce']['total'] = $total * 100;
