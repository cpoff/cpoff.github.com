...
function get_form_item_view_screen_content($properties) {
    ...
    if (COLLECTION == 'b') {
        ...
        $properties = get_page_type_properties($current_page_id, 'form item view', 'b');
    ...
    // if the user came from the control panel then that means that the user just clicked directly to this form item view from the control panel,
    // so just output the layout as it is without converting variables to values
    if ((isset($_GET['from']) == TRUE) && ($_GET['from'] == 'control_panel')) {
        $output =
            '<div class="software_notice">
                <img style="float: none; display: inline-block; height: 1em; width: 1em; min-height: 16px; min-width: 16px;" class="icon" title="" alt="Notice" src="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/images/icon_notice.png">
                <div style="display: inline; vertical-align: top;">
                    <span class="description">Notice:</span> 
                    <span class="notice">This page will only show data if a reference code is passed to it via the link to this page.<br />(Typically from a Form List View page).</span>
                </div>
            </div>' .
            $layout;
    ...
    // else the user did not come from the control panel, so prepare output differently
        ...
        LEFT JOIN submitted_form_info ON ((forms.id = submitted_form_info.submitted_form_id) AND (submitted_form_info.page_id = '$current_page_id'))
        ...
        if (($editable == false) && (mysqli_num_rows($result) == 0)) {
            ...
            // Otherwise if a pretty URL is being used to access this page, then output error for that.
                ...
                output_error('The submitted form could not be found. Please check that you have entered the correct address. The address for the submitted form might have changed, or the submitted form might have been deleted. <a href="javascript:history.go(-1)">Go back</a>.');

            // Otherwise this page is being called through an ugly URL, so output error for that.
            } else {
                if ($_GET['r']) {
                    output_error('The submitted form could not be found. The reference code might be invalid or the submitted form might have been deleted. <a href="javascript:history.go(-1)">Go back</a>.');
                } else {
                    output_error('There was no reference code passed to this page, so a submitted form could not be found. You may browse to a submitted form from a form list view page. <a href="javascript:history.go(-1)">Go back</a>.');
                }
            }
        }
        ...
        if (($edit_access == true) && ($_GET['edit_submitted_form'] == 'true')) {
            ...
            $output = 
                ...
                <form' . $output_enctype . ' action="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/edit_submitted_form.php" method="post" style="margin: 0px">
                    ...
                    <input type="hidden" name="form_item_view_page_id" value="' . $current_page_id . '" />
        ...
        // else user has not chosen to edit submitted form, so just output layout with values
            ...
            if (db_value("SELECT COUNT(*) FROM submitted_form_info WHERE (submitted_form_id = '" . $submitted_form['id'] . "') AND (page_id = '" . $current_page_id . "')") == 0) {
                db(
                ...
                $output_auto_registration =
                    ...
                    <div class="account data">
                        ...
                        <p>
                            ...
                            Temporary Password: ' . h($_SESSION['software']['custom_form_auto_registration'][$submitted_form['id']]['password']) . '
                        </p>
                    </div>';
            }
