...
var software = {
    ...
    // Deals with updating state field pick list options or text box field
    // when a country is selected based on whether country has states or not.
    init_country: function(properties) {
        ...
        // When the user changes the country pick list, then update the state field.
        country_pick_list.change(function() {
            ...
            if (selected_country && countries[selected_country]) {
    ...
    init_custom_arrival_date: function(id) {
        // Init the date-picker.
        software_$('#custom_arrival_date_' + id).datepicker({
            dateFormat: date_picker_format
        });
        ...
        software_$('#arrival_date_' + id).click(function() {
            if (software_$(this).attr('checked') == 'checked') {
                software_$('#custom_arrival_date_' + id).focus();
            }
        });
        ...
        software_$('#custom_arrival_date_' + id).change(function() {
            if (software_$.trim(software_$(this).val()).length != 0) {
                software_$('#arrival_date_' + id).attr('checked', 'checked');
            }
        });
    },
    ...
    init_payment_method: function() {
        ...
        purchase_now_button.attr('data-label', purchase_now_button.html());
        ...
        function update() {
            ...
            if (payment_method == 'Credit/Debit Card') {
                ...
                purchase_now_button.html(purchase_now_button.attr('data-label'));
            ...
            if (payment_method == 'PayPal Express Checkout') {
                purchase_now_button.html(purchase_now_button.attr('data-paypal-label'));
            } else {
                purchase_now_button.html(purchase_now_button.attr('data-label'));
            }
    ...
    init_product_attributes: function(properties) {
        ...
        // Hide various content that was outputted by the server, as soon as possible,
        // because we are going to fade in the proper
        // content once we figure out the matched products.
        software_$('.software_catalog_detail .image').hide();
        software_$('.software_catalog_detail .short_description').html('').hide();
        software_$('.software_catalog_detail .price').html('').hide();
        ...
        // Add change events to all attribute pick lists.
        software_$('.attribute_row select').change(function() {
            var attribute_id = this.name.split('_')[1];
            var value = software_$(this).val();
            
            // Loop through the attributes in order to remember whether
            // the user has unselected the pick list or not for this attribute.
            // We do this so later we will know whether we should change
            // the default selected option when there is only one option or not.
            software_$.each(attributes, function(index, attribute) {
                // If this is the not the attribute that was just changed,
                ...
                // We are done now, so we can break out of the loop.
                return false;
            });
        ...
        function update(number_of_updates) {
            ...
            software_$.each(attributes, function(index, attribute) {
                ...
                var selected_option = software_$('.attribute_' + attribute.id + ' select').val();
            ...
            var image = software_$('.software_catalog_detail .image');
            ...
            var short_description_container = software_$('.software_catalog_detail .short_description');
            ...
            var price_container = software_$('.software_catalog_detail .price');
