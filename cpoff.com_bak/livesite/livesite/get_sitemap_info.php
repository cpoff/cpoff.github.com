...
function get_sitemap_info() {
    ...
    // Get the short links with recently modified ones first.
    ...
    // Loop through the short links in order to add them to array.
    while ($row = mysqli_fetch_assoc($result)) {
        // If a short link for this page has not already been added, then add it.
        if (isset($short_links[$row['page_id']]) == FALSE) {
            $short_links[$row['page_id']] = $row['name'];
        }
    }
    ...
    foreach ($pages as $page) {
        ...
        if (in_array($page['folder_id'], $public_folder_ids) == TRUE) {
            ...
            if ($short_links[$page['id']] != '') {
                $output_path = h(PATH . encode_url_path($short_links[$page['id']]));
                ...
                if (
                    ...
                    if (count($submitted_forms) > 0) {
                        ...
                        if ($short_links[$form_item_view_page_id] != '') {
                            $output_ugly_path = h(PATH . encode_url_path($short_links[$form_item_view_page_id]));
                            
                        // else there is not a short link for the form item view, so use the page name for the path
                        } else {
            ...
            if ((in_array($calendar_event_view_folder_id, $public_folder_ids) == TRUE) && ($calendar_event_view_sitemap == 1)) {
                ...
                if (count($calendar_events) > 0) {
                    ...
                    if ($short_links[$calendar_event_view_page_id] != '') {
                        $output_path = h(PATH . encode_url_path($short_links[$calendar_event_view_page_id]));
                        
                    // else there is not a short link for the calendar event view, so use the page name for the path
                    } else {
