...
function get_shipping_address_and_arrival($properties) {
    ...
    if ($form->field_in_session('first_name') == false) {
        ...
        // if user is logged in, see if we can find recipient in the address book
        if (isset($_SESSION['sessionusername'])) {
            ...
            // if user is found, see if we can find a recipient in the address book
            if ($user_id) {
                $query = "SELECT * FROM address_book WHERE user = $user_id AND ship_to_name = '" . escape($ship_to_name) . "'";
                $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
    ...
    if ($layout_type == 'system') {
        ...
        // get all states which we will use in a couple of places below
        $query =
            "SELECT
                states.id,
                states.name,
        ...
        foreach ($states as $state) {
            $output_states_array .= 'states[' . $index . '] = new Array("' . $state['name'] . '", "' . $state['code'] . '", "' . $state['country_code'] . '");' . "\n";
            $index++;
        }
        ...
        // set blank first option for the county selection drop-down
        $country_options[''] = '';
        
        // loop through the countries in order to prepare pick list options
        ...
        $output_shipping_address_fields =
            '<table>
                ...
                <tr>
                    ...
                    <td><div id="state_container">' . $form->output_field(array('type'=>'text', 'id'=>'state', 'name'=>'state', 'size'=>'40', 'maxlength'=>'50', 'class'=>'software_input_text')) . '</div></td>
                </tr>
                <tr>
                    <td>Zip / Postal Code*</td>
                    <td>' . $form->output_field(array('type'=>'text', 'id'=>'zip_code', 'name'=>'zip_code', 'maxlength'=>'50', 'class'=>'software_input_text', 'required' => 'true')) . '</td>
                </tr>
                <tr>
                    <td>Country*</td>
                    <td>' . $form->output_field(array('type'=>'select', 'id'=>'country', 'name'=>'country', 'options'=>$country_options, 'class'=>'software_select', 'required' => 'true', 'onchange'=>'change_states(\'country\', \'state\')')) . '<script type="text/javascript" language="JavaScript 1.2">change_states(\'country\', \'state\', true);</script></td>
                </tr>
                ' . $output_address_type_row . '
                <tr>
                    <td>Phone</td>
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            if ($row['custom']) {
                $custom_field =
                    ...
                    <script>software.init_custom_arrival_date(' . $row['id'] . ')</script> ';
            } else {
                $custom_field = '';
            }
    ...
    // Otherwise the layout is custom.
        ...
        $form->set('zip_code', 'required', true);
        ...
        if (
            ...
            foreach ($custom_arrival_dates as $custom_arrival_date) {
                $system .= '<script>software.init_custom_arrival_date(' . $custom_arrival_date . ')</script>';
            }
        }
