...
function get_update_address_book($properties = array()) {
    ...
    if ($layout_type == 'system' || !$layout_type) {
        ...
        $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
        while ($row = mysqli_fetch_assoc($result)) {
            $country_options[$row['name']] = $row['code'];
        }
            ...
            <form action="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/update_address_book.php" method="post" class="software">
                ...
                <table style="margin-bottom: 15px">
                    ...
                    <tr>
                        ...
                        <td><div id="state_container">' . $form->output_field(array('type'=>'text', 'name'=>'state', 'id'=>'state', 'maxlength'=>'50', 'class'=>'software_input_text')) . '</div></td>
                    </tr>
                    <tr>
                        <td>Zip / Postal Code*</td>
                        <td>' . $form->output_field(array('type'=>'text', 'name'=>'zip_code', 'maxlength'=>'50', 'class'=>'software_input_text')) . '</td>
                    </tr>
                    <tr>
                        <td>Country*</td>
                        <td>' . $form->output_field(array('type'=>'select', 'name'=>'country', 'id'=>'country', 'options'=>$country_options, 'class'=>'software_select', 'onchange'=>'change_states(\'country\', \'state\')')) . '<script type="text/javascript" language="JavaScript 1.2">change_states(\'country\', \'state\', true);</script></td>
                    </tr>
                    ' . $output_address_type_row . '
                    <tr>
                        <td>Phone</td>
