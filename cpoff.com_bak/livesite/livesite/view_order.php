        ...
        if ($payment_method == 'Credit/Debit Card') {
            ...
            $output_credit_debit_card_information =
                ...
                <tr>
                    ...
                    <td>' . $card_verification_number . '</td>
                </tr>';
            ...
            if ($ship_tos[$key]['ship_to_id'] > 0) {
                ...
                if ($status != 'incomplete') {
                    ...
                    foreach ($shipping_tracking_numbers as $shipping_tracking_number) {
                        ...
                        $shipping_tracking_url = get_shipping_tracking_url($shipping_tracking_number['number']);
                ...
                $output_order_details .=
                    ...
                    <tr>
                        ...
                        <td>' . $ship_tos[$key]['shipping_method_code'] . '</td>
                        <td>
                            ' . $output_shipping_tracking_numbers . '
                            ' . $output_shipping_tracking_numbers_field . '
                        </td>
...
// else the form has been submitted
    ...
    if ($liveform->get_field_value('submit_delete') == 'Delete') {
        ...
        $query = "SELECT order_number FROM orders WHERE id = '" . escape($liveform->get_field_value('id')) . "'";
        $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
        $row = mysqli_fetch_assoc($result);
        $order_number = $row['order_number'];
        
        // delete order
        $query = "DELETE FROM orders WHERE id = '" . escape($liveform->get_field_value('id')) . "'";
        $result = mysqli_query(db::$con, $query) or output_error('Query failed.');

        // delete order items for order
        $query = "DELETE FROM order_items WHERE order_id = '" . escape($liveform->get_field_value('id')) . "'";
        $result = mysqli_query(db::$con, $query) or output_error('Query failed.');

        // delete ship tos for order
        $query = "DELETE FROM ship_tos WHERE order_id = '" . escape($liveform->get_field_value('id')) . "'";
        $result = mysqli_query(db::$con, $query) or output_error('Query failed.');

        db("DELETE FROM order_item_gift_cards WHERE order_id = '" . escape($liveform->get_field_value('id')) . "'");

        db("DELETE FROM applied_gift_cards WHERE order_id = '" . escape($liveform->get_field_value('id')) . "'");

        // Delete custom billing, shipping, and product form data for order.
        $query = "DELETE FROM form_data WHERE (order_id = '" . escape($liveform->get_field_value('id')) . "') AND (order_id != '0')";
        $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
        
        // delete shipping tracking numbers for order
        $query = "DELETE FROM shipping_tracking_numbers WHERE order_id = '" . escape($liveform->get_field_value('id')) . "'";
        $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
        ...
        db(
            ...
            WHERE
                ...
                AND (email_campaigns.order_id = '" . e($liveform->get_field_value('id')) . "')");

        // if order was a completed order and had an order number, include order number in log message
        if ($order_number) {
            $order_number = " ($order_number)";
        }

        log_activity('order' . $order_number . ' was deleted', $_SESSION['sessionusername']);
    ...
    // else the user selected to save the order
        ...
        // Get all ship tos for this order in order to save shipping info.
        $query =
            "SELECT id
            FROM ship_tos
            WHERE order_id = '" . escape($liveform->get_field_value('id')) . "'";
        $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
        $ship_tos = mysqli_fetch_items($result);
        
        // delete existing shipping tracking numbers for this order, so we can insert new ones
        $query = "DELETE FROM shipping_tracking_numbers WHERE order_id = '" . escape($liveform->get_field_value('id')) . "'";
        $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
        
        // Loop through ship tos in order to save shipping info.
        foreach ($ship_tos as $ship_to) {

            $ship_date = prepare_form_data_for_input($liveform->get('ship_to_id_' . $ship_to['id'] . '_ship_date'), 'date');
            $delivery_date = prepare_form_data_for_input($liveform->get('ship_to_id_' . $ship_to['id'] . '_delivery_date'), 'date');
            ...
            // if at least one shipping tracking number was added for this ship to then them for this ship to
            if ($liveform->get_field_value('ship_to_id_' . $ship_to['id'] . '_tracking_numbers') != '') {
                // load all shipping tracking numbers into an array
                $shipping_tracking_numbers = explode("\n", $liveform->get_field_value('ship_to_id_' . $ship_to['id'] . '_tracking_numbers'));
                
                // loop through all shipping tracking numbers in order to add them to database
                foreach ($shipping_tracking_numbers as $shipping_tracking_number) {
                    $shipping_tracking_number = trim($shipping_tracking_number);
                    
                    // if the shipping tracking number is not blank, then add it to the database
                    if ($shipping_tracking_number != '') {
                        $query =
                            ...
                            VALUES (
                                ...
                                '" . $ship_to['id'] . "',
                                '" . escape($shipping_tracking_number) . "')";
                        $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
                    }
                }
            }
            ...
            // loop through order items in order to update shipped quantity
            foreach ($order_items as $order_item) {
                $show_shipped_quantity = 0;
                $shipped_quantity = 0;
                
                // if shipped quantity was not blank, then mark it to be shown
                if ($liveform->get_field_value('order_item_id_' . $order_item['id'] . '_shipped_quantity') != '') {
                    $show_shipped_quantity = 1;
                    $shipped_quantity = $liveform->get_field_value('order_item_id_' . $order_item['id'] . '_shipped_quantity');
                }
            
                // update shipped quantity for order item
                $query =
                    "UPDATE order_items
                    SET
                        show_shipped_quantity = '" . $show_shipped_quantity . "',
                        shipped_quantity = '" . escape($shipped_quantity) . "'
                    WHERE id = '" . $order_item['id'] . "'";
                $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
            }
        }

        db("UPDATE orders SET last_modified_timestamp = UNIX_TIMESTAMP() WHERE id = '" . escape($liveform->get_field_value('id')) . "'");
        ...
        log_activity('order (' . $order_number . ') was modified', $_SESSION['sessionusername']);
