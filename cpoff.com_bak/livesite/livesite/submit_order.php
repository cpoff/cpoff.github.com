...
// submits an order for the order preview and express order features
function submit_order($type) {
    ...
    // get first ship to id that is not complete, if one exists, so we can forward user to correct shipping address & arrival page
    $query = "SELECT id FROM ship_tos WHERE (order_id = '" . $_SESSION['ecommerce']['order_id'] . "') AND (complete = '0') ORDER BY id LIMIT 1";
    $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
    
    // if an incomplete ship to was found, add error and forward user to shipping address & arrival page for ship to
    if (mysqli_num_rows($result) > 0) {
        $row = mysqli_fetch_assoc($result);
        
        $liveform_shipping_address_and_arrival = new liveform('shipping_address_and_arrival');
        $liveform_shipping_address_and_arrival->mark_error('incomplete_ship_tos_error', 'Your order could not be submitted because the shipping information for one or more recipients is incomplete. Please update the shipping information below and then try to submit your order again.');
        
        header('Location: ' . URL_SCHEME . HOSTNAME . PATH . encode_url_path(get_page_name($_SESSION['ecommerce']['shipping_address_and_arrival_page_id'])). '?ship_to_id=' . $row['id']);
        exit();
    }
    
    // if the customer came from the order preview screen, then verify that billing information is complete
    // we don't have to do this for the express order screen because are already going to verify the billing information for that screen because billing fields appear on that screen
    if ($type == 'order preview') {
        ...
        // if billing information is not complete, add error and forward user to billing information screen
        if ($billing_complete == 0) {
            $liveform_billing_information = new liveform('billing_information');
            $liveform_billing_information->mark_error('incomplete_billing_error', 'Your order could not be submitted because the billing information is incomplete. Please update the billing information and then try to submit your order again.');
            
            header('Location: ' . URL_SCHEME . HOSTNAME . PATH . encode_url_path(get_page_name($_SESSION['ecommerce']['billing_information_page_id'])));
            exit();
        }
    }
    
    // get user id if user is logged in.  we will use this in various areas below
    $query = "SELECT user_id FROM user WHERE user_username = '" . escape($_SESSION['sessionusername']) . "'";
    $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
    $row = mysqli_fetch_assoc($result);
    $user_id = $row['user_id'];
    
    // if the user is logged in, look for contact in user record
    if ($user_id) {
        $query = "SELECT user_contact FROM user WHERE user_id = '$user_id'";
        $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
        $row = mysqli_fetch_assoc($result);
        $contact_id = $row['user_contact'];
        
    // else the user is not logged in, so use contact in order if contact is not connected to a user
    } else {
        // get contact id for this order
        $query = "SELECT contact_id FROM orders WHERE id = '" . $_SESSION['ecommerce']['order_id'] . "'";
        $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
        $row = mysqli_fetch_assoc($result);
    ...
    // check to see if contact still exists
    ...
    if ($_GET['mode'] != 'paypal_express_checkout_return') {
        ...
        if ($type == 'express order') {
            ...
            // find out if state should be required depending on whether there are states for selected country
            $query = "SELECT states.id
                     FROM states
                     LEFT JOIN countries ON countries.id = states.country_id
                     WHERE countries.code = '" . escape($liveform->get_field_value('billing_country')) . "'
                     ORDER BY states.name";
            $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
            // if a state was found for selected country, require state field
            if (mysqli_num_rows($result) > 0) {
            ...
            if ($form == 1) {
                submit_custom_form(array('page_type' => 'express order'));
            }
            ...
            // verify that the address is a US address
            $address_verified = verify_us_address($liveform, 'billing', $address_verified);
            ...
            // If the user is logged in and the user selected that he/she
            ...
            if (USER_LOGGED_IN && !$liveform->get_field_value('update_contact')) {
                $_SESSION['software']['update_contact'] = false;
            } else {
                unset($_SESSION['software']['update_contact']);
            }

            // If the user is not logged in or if the user is logged in and wants
    ...
    if ($_GET['mode'] != 'paypal_express_checkout_return') {
        ...
        if (($nonrecurring_transaction == TRUE) || ($recurring_transaction == TRUE)) {
            ...
            if (
                ...
                $liveform->validate_required_field('card_type', 'Card Type is required.');
                ...
                $liveform->validate_required_field('card_verification_number', 'Verification Number is required.');
                
                // if there is not already an error for the card number field
                // and there is not an error for the credit card type field,
                // validate credit card number
                if (($liveform->check_field_error('card_number') == false) && ($liveform->check_field_error('card_type') == false)) {
                    ...
                    switch ($liveform->get_field_value('card_type')) {
                        case 'American Express':
                            if (($card_number_length == 15) && ((($card_number_prefix >= 3400) && ($card_number_prefix <= 3499)) || (($card_number_prefix >= 3700) && ($card_number_prefix <= 3799)))) {
                        ...
                        case 'Diners Club':
                            if (($card_number_length == 14) && ((($card_number_prefix >= 3000) && ($card_number_prefix <= 3059)) || (($card_number_prefix >= 3600) && ($card_number_prefix <= 3699)) || (($card_number_prefix >= 3800) && ($card_number_prefix <= 3889)))) {
                                $valid_credit_card = true;
                            } else {
                                $valid_credit_card = false;
                            }
                        ...
                        case 'Discover Card':
                            if (($card_number_length == 16) && ($card_number_prefix == 6011)) {
                                $valid_credit_card = true;
                            } else {
                                $valid_credit_card = false;
                            }
                        ...
                        case 'MasterCard':
                            if (($card_number_length == 16) && ($card_number_prefix >= 5100) && ($card_number_prefix <= 5599)) {
                                $valid_credit_card = true;
                            } else {
                                $valid_credit_card = false;
                            }
                        ...
                        case 'Visa':
                            if ((($card_number_length == 16) || ($card_number_length == 13)) && ($card_number_prefix >= 4000) && ($card_number_prefix <= 4999)) {
                                $valid_credit_card = true;
                            } else {
                                $valid_credit_card = false;
                            }
                    ...
                    if ($valid_credit_card == false) {
                        $liveform->mark_error('card_number', 'The credit/debit card number is invalid. Please verify that Card Type and Card Number are correct.');
                    }
                }

                // if there is not already an error for the expiration month and expiration year, check to see if expiration date has expired
                if (($liveform->check_field_error('expiration_month') == false) && ($liveform->check_field_error('expiration_year') == false)) {
                    // if the selected expiration year is less than the current year
                    if ($liveform->get_field_value('expiration_year') < date('Y')) {
                        $valid_expiration_date = false;
                        
                    // else if the selected expiration year is equal to the current year
                    } elseif ($liveform->get_field_value('expiration_year') == date('Y')) {
                        // if the selected expiration month is greater than or equal to current month
                        if ($liveform->get_field_value('expiration_month') >= date('m') ) {
                            $valid_expiration_date = true;
                        
                        // else the selected expiration month is less than the current month
                        } else {
                            $valid_expiration_date = false;
                        }
                   
                    // else the selected expiration year is greater than the current year
                    } else {
                        $valid_expiration_date = true;
                    }
                    
                    // if the expiration date is not valid, mark error
                    if ($valid_expiration_date == false) {
                        $liveform->mark_error('expiration_month', 'The expiration date has passed.');
    ...
    // if there is not already an error or notice and the total is not equal to the total that was last displayed to the user,
    // then we need to show the correct total to the user before the order is submitted, so add notice
    // we have to do this in case gift card balances have changed or tax has changed (e.g. after billing info is submitted on express order screen)
    // this issue could also likely occur after a user was forwarded to submit payment at PayPal but that could take a while.
    // the round functions had to be added because PHP would sometimes consider the same integers to not be equal for some reason
    if (
        ($liveform->check_form_errors() == FALSE)
        && ($liveform->check_form_notices() == FALSE)
        &&
        (
            (
                ($surcharge == 0)
                && (round($total) != round($_SESSION['ecommerce']['total']))
            )
            ||
            (
                ($surcharge > 0)
                && (round($total) != round($_SESSION['ecommerce']['total_with_surcharge']))
            )
        )
    ) {
        $liveform->add_notice('The order was not submitted, because the total has been updated. Please review the new total and submit the order again.');
    }

    // If hooks are enabled and there is pre-save hook code, then run it.
    if ((PHP_REGIONS === true) && ($pre_save_hook_code != '')) {
                ...
                switch (ECOMMERCE_PAYMENT_GATEWAY) {
                    case 'Authorize.Net':
                        ...
                        $transaction_values =
                            array(
                                ...
                                'x_exp_date' => $liveform->get_field_value('expiration_month') . '/' . $liveform->get_field_value('expiration_year'),
                                        ...
                                        <Consumer>
                                            <PaymentMech>
                                                <CreditCard>
                                                    ...
                                                    <Expires DataType="ExpirationDate" Locale="840">' . h($liveform->get_field_value('expiration_month')) . '/' . h($liveform->get_field_value('expiration_year')) . '</Expires>
                        ...
                        $transaction_xml =
                            '<order>
                                ...
                                <creditcard>
                                    ...
                                    <cardexpmonth>' . h($liveform->get_field_value('expiration_month')) . '</cardexpmonth>
                                    <cardexpyear>' .  h(mb_substr($liveform->get_field_value('expiration_year'), -2)) . '</cardexpyear>
                    ...
                    case 'PayPal Payflow Pro':
                        ...
                        $transaction_values = array(
                            ...
                            'ACCT'    => $card_number,
                            'EXPDATE' => $liveform->get_field_value('expiration_month') . mb_substr($liveform->get_field_value('expiration_year'), -2),
                    ...
                    case 'PayPal Payments Pro':
                        ...
                        $transaction_values = array(
                            ...
                            'ACCT' => $card_number,
                            'EXPDATE' => $liveform->get_field_value('expiration_month') . $liveform->get_field_value('expiration_year'),
                            'CVV2' => $liveform->get_field_value('card_verification_number'),
                            'EMAIL' => $billing_email_address,
                            'FIRSTNAME' => $billing_first_name,
                            'LASTNAME' => $billing_last_name,
                    ...
                    case 'Sage':
                        ...
                        $transaction_values = array(
                            ...
                            'C_exp' => $liveform->get_field_value('expiration_month') . mb_substr($liveform->get_field_value('expiration_year'), -2), // mmyy
                    ...
                    case 'Stripe':
                        ...
                        try {
                            $response = Stripe_Charge::create(array(
                                ...
                                'card' => array(
                                    'number' => $card_number,
                                    'exp_month' => $liveform->get_field_value('expiration_month'),
                                    'exp_year' => $liveform->get_field_value('expiration_year'),
    ...
    // If a special offer code was added to this order,
    // then check if code is a single-use key code.
    if ($special_offer_code != '') {
        $key_code = db_item(
            ...
            FROM key_codes
            WHERE
                (code = '" . e($special_offer_code) . "')
                AND (single_use = '1')");

        // If a single-use key code was found, then disable key code,
        // and update code in order to contain offer code instead.
        if ($key_code) {
            // Disable key code.
            db(
                "UPDATE key_codes
            ...
            // Prepare to update special offer code in order to contain offer code
            // instead of key code.  We do this because for single-use key codes,
            // it is likely more valuable to the organization to be able to track
            // which offer was used on the order, rather than the key code.
    ...
    // then deal with auto-registration.
    if (($auto_registration == 1) && (!USER_LOGGED_IN)) {
        ...
        $user_id = db_value("SELECT user_id FROM user WHERE user_email = '" . escape($billing_email_address) . "'");
        ...
        if (!$user_id) {
            ...
            log_activity('user was auto-logged in by order auto-registration', $_SESSION['sessionusername']);
    ...
    if ($liveform->get_field_value('payment_method') == 'Credit/Debit Card') {
        ...
        $sql_credit_debit_card_values =
            "card_type = '" . escape($liveform->get_field_value('card_type')) . "',
            card_number = '" . escape($processed_card_number) . "',
            expiration_month = '" . escape($liveform->get_field_value('expiration_month')) . "',
            expiration_year = '" . escape($liveform->get_field_value('expiration_year')) . "',
            cardholder = '" . escape($liveform->get_field_value('cardholder')) . "',
                            ...
                            "<name>" . h($profile_name) . "</name>".
                                ...
                                "<payment>".
                                    "<creditCard>".
                                        ...
                                        "<expirationDate>" . h($liveform->get_field_value('expiration_year')) . "-" . h($liveform->get_field_value('expiration_month')) . "</expirationDate>".
                                    "</creditCard>".
                                "</payment>".
                                "<order>".
                        ...
                        if ($payment_gateway_error_message == '') {
                            ...
                            if ($payment_gateway_error_message == '') {
                                ...
                                $transaction_values = array(
                                    ...
                                    'CARD_EXP_MONTH' => $liveform->get_field_value('expiration_month'),
                                    'CARD_EXP_YEAR' => mb_substr($liveform->get_field_value('expiration_year'), -2),
    ...
    // If shipping is enabled, then update ship date & delivery date for all ship tos.
    if (ECOMMERCE_SHIPPING == true) {
        ...
        $ship_tos = db_items(
            ...
            WHERE
                (ship_tos.order_id = '" . $_SESSION['ecommerce']['order_id'] . "')");

        // Loop through the ship tos in order to update dates.
        foreach ($ship_tos as $ship_to) {
            ...
            if ($ship_to['arrival_date'] == '0000-00-00') {
                ...
                // If the order time was before the end of day time, then the order start date is the order date.
                if ($order_time < ECOMMERCE_END_OF_DAY_TIME) {
                    $order_start_date = date('Y-m-d', $order_date);
                    
                // Otherwise the order time is after end of day time,
                // so the order start date is the day after the order date.
                } else {
                    $order_start_date = date('Y-m-d', strtotime('+1 day', $order_date));
                }

                // Get the maximum preparation time by looking for the product in this ship to
                // that has the greatest preparation time.
                $preparation_time = db_value(
                    "SELECT MAX(products.preparation_time)
                    FROM order_items
                    LEFT JOIN products ON order_items.product_id = products.id
                    WHERE order_items.ship_to_id = '" . $ship_to['id'] . "'");

                // If the preparation time for the product is 0,
                // then the ship date is the order start date.
                if ($preparation_time == 0) {
                    $ship_date = $order_start_date;

                // Otherwise the preparation time for the product is not 0,
                // so add preparation time to order start date to calculate ship date.
                } else {
                    $ship_date = date('Y-m-d', strtotime('+' . $preparation_time . ' day', strtotime($order_start_date)));
                }
    ...
    if ((PHP_REGIONS === true) && ($post_save_hook_code != '')) {
