...
if ($user['role'] < 3 || no_acl_check($user['id'])) {
    ...
    if (($user['role'] < 3) || (check_folder_access_in_array($page_folder, $folders_that_user_has_access_to) == true)) {
        ...
        if ($user['role'] <= 1) {
            ...
            $output_edit_page_style_button .= '<a href="edit_' . $style_type . '_style.php?id=' . h($style_id) . '&send_to=' . h(urlencode($_GET['send_to'])) . '" target="_parent"' . $output_title . '>' . $output_edit_page_style_button_label . '</a>';
        }
        ...
        if (($user['role'] < '3') || ($user['create_pages'] == TRUE)) {
            $output_duplicate_page_button = '<a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/duplicate_page.php?id=' . h($_GET['page_id']) . get_token_query_string_field() . '" target="_parent" title="Duplicate Page">Duplicate</a>';
        }
        ...
        if ($user['role'] < 3) {
            ...
            if (isset($_SESSION['software']['preview_theme_id']) == TRUE) {
                ...
                $output_preview_style_set_button = '<a href="preview_style.php?mode=set&amp;page_id=' . $page_id . '&amp;send_to=' . h(urlencode($_GET['send_to'])) . get_token_query_string_field() . '" target="_parent" title="' . $output_preview_style_set_button_title . '">Set</a>';
                ...
                if (
                    ...
                    if ($is_system_theme == TRUE) {
                        $output_edit_theme_link = '<a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/theme_designer.php?id=' . h($selected_file_id) . '&send_to=' . h(urlencode($_GET['send_to'])) . '&clear_theme_designer_session=true&page_to_preview_id=' . $page_id . '" target="_parent" title="Edit Theme">Edit</a>';
                        
                    // else output the edit css link
                    } else {
                        $output_edit_theme_link = '<a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/edit_theme_css.php?id=' . h($selected_file_id) . '&send_to=' . h(urlencode($_GET['send_to'])) . '" target="_parent" title="Edit Theme">Edit</a>';
                    }
                }
                ...
                $output_theme_pick_list =
                    ...
                    $output_edit_theme_link .
                    '<a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/preview_theme.php?mode=cancel&amp;send_to=' . h(urlencode($_GET['send_to'])) . get_token_query_string_field() . '" target="_parent" title="Close Preview">x</a>';
                
            // else the user is not previewing a stylesheet, so output preview themes link
            } else {
                $output_preview_theme_link = '<a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/preview_theme.php?mode=preview&amp;send_to=' . h(urlencode($_GET['send_to'])) . get_token_query_string_field() . '" target="_parent" title="Preview Page Styles &amp; Themes">Preview Themes</a>';
            }
        }
        ...
        $output_button_bar =
            '<div id="button_bar">
                ...
                '</div>
                <div class="center">' . 
                    '<a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/edit_page.php?id=' . h($_GET['page_id']) . '&amp;send_to=' . h(urlencode($_GET['send_to'])) . '" target="_parent" title="Edit Page Properties">Edit Page Properties</a>' .
                    $output_duplicate_page_button .
                '</div>
