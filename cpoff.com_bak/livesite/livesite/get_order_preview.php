...
function get_order_preview($properties) {
    ...
    if ($layout_type == 'system') {
        ...
        // else the total is 0 or there is an active payment method, so prepare to output purchase now button
            ...
            // then use PayPal Express Checkout submit button label
            if ((($show_paypal_express_checkout_payment_method == TRUE) && ($show_credit_debit_card_payment_method == FALSE)) || ($form->get_field_value('payment_method') == 'PayPal Express Checkout')) {
                $output_submit_button_label = $paypal_express_checkout_submit_button_label;
                
            // else PayPal Express Checkout payment method is not selected, so use standard submit button label
            } else {
                $output_submit_button_label = h($standard_submit_button_label);
            }
            
            $output_purchase_now_button =
                '<div class="mobile_margin_top" style="text-align: right">
                    <input type="submit" name="submit_purchase_now" id="submit_purchase_now" value="' . $output_submit_button_label . '" class="software_input_submit_primary purchase_button" />
                </div>';
        }
        ...
        $output =
            ...
            <form action="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/order_preview.php" method="post">
                ...
                <table class="products" style="width: 100%; margin-bottom: 15px">
                    ...
                    ' . $output_gift_card_discount . '
                    ' . $output_surcharge_rows . '
                    <tr class="order_totals data" id="software_total_row">
                        <td class="mobile_left" colspan="4" style="text-align: right"><strong>Total Due:</strong></td>
