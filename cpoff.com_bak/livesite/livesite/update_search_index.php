...
echo
    ...
    <html lang="en">
        ...
        <body class="pages">
            <div id="subnav" style="position: fixed; top: 0; left: 0; width: 100%; height: 7em; z-index: 999;">
...
// Get short links so we can later use short link for URL if one exists.
// We are getting the short links now for performance reasons, in order to avoid
// generating a ton of queries.  Get the short links with recently modified ones first.
// We will ignore duplicate short links for the same page later below.
$temp_short_links = db_items(
    "SELECT
        page_id,
        name
    FROM short_links
...
// Loop through the temp short links in order to store unique ones.
foreach ($temp_short_links as $temp_short_link) {
    // If a short link for this page has not already been added, then add it.
    if (isset($short_links[$temp_short_link['page_id']]) == false) {
        $short_links[$temp_short_link['page_id']] = $temp_short_link['name'];
    }
}
...
foreach ($pages as $page) {
    ...
    if ($short_links[$page['id']] != '') {
        $url = encode_url_path($short_links[$page['id']]);
        
    // Otherwise there is not a short link for this page, so use the page name for the path.
    } else {
            ...
            if (count($calendar_ids) > 0) {
                ...
                if (
                    ...
                    if (count($calendar_events) > 0) {
                        // If there is a short link for the calendar event view, then use that for the path.
                        if ($short_links[$calendar_event_view_page_id] != '') {
                            $path = encode_url_path($short_links[$calendar_event_view_page_id]);
                            
                        // Otherwise use the page name for the path.
                        } else {
        ...
        case 'form list view':
            ...
            if (
                ...
                if (count($submitted_forms) > 0) {
                    ...
                    // If there is a short link for the form item view, then use that for the path.
                    if ($short_links[$form_item_view_page_id] != '') {
                        $ugly_path = encode_url_path($short_links[$form_item_view_page_id]);
