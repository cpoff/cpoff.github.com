    ...
    // else this order item is not a donation
        ...
        // else if quantity is a positive integer and new quantity is different from old quantity, update quantity for product
            ...
            if ($ship_to_id && ($complete == 1)) {
                require_once(dirname(__FILE__) . '/shipping_cost.php');
...
if (isset($_POST['special_offer_code']) == TRUE) {
    $sql_affiliate_code = "";
    
    $liveform->assign_field_value('special_offer_code', $_POST['special_offer_code']);
    
    // if a special offer code was entered
    if ($liveform->get_field_value('special_offer_code') != '') {
        $offer_code = get_offer_code_for_special_offer_code($liveform->get_field_value('special_offer_code'));
        
        // get shopping cart label and special offer code label, in case there is an error
        $query =
            "SELECT
                shopping_cart_label,
        ...
        $query =
            "SELECT
                ...
                offers.end_date,
                offers.scope,
                required_products.id AS offer_rule_required_product_id
            FROM offers
            LEFT JOIN offer_rules ON offers.offer_rule_id = offer_rules.id
            LEFT JOIN products AS required_products ON offer_rules.required_product_id = required_products.id
        ...
        if (mysqli_num_rows($result) > 0) {
                ...
                if (
                    ...
                    foreach ($offers as $offer) {
                        ...
                        if (
                            ...
                            || ($offer['scope'] == 'order')
                            || ($offer['offer_rule_required_product_id'] == '')
                        ) {
            ...
            // else an active offer does not exist, so add error
                ...
                $liveform->mark_error('special_offer_code', 'The ' . h($special_offer_code_label) . ' that you entered (' . h($liveform->get_field_value('special_offer_code')) . ') is not available at this time. Please try a different code.');
                $liveform->assign_field_value('special_offer_code', '');
            }
        
        // else an offer was not found, so prepare error
        } else {
            $liveform->mark_error('special_offer_code', 'The ' . h($special_offer_code_label) . ' that you entered (' . h($liveform->get_field_value('special_offer_code')) . ') could not be found. Please try a different code.');
            $liveform->assign_field_value('special_offer_code', '');
        }
        ...
        if (AFFILIATE_PROGRAM == TRUE) {
            $query = "SELECT id FROM contacts WHERE (affiliate_code = '" . escape($liveform->get_field_value('special_offer_code')) . "') AND (affiliate_approved = '1')";
            $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                if (VISITOR_TRACKING == TRUE) {
                    $_SESSION['software']['affiliate_code'] = $liveform->get_field_value('special_offer_code');
                    setcookie('software[affiliate_code]', $liveform->get_field_value('special_offer_code'), time() + 315360000, '/');
                    
                    $query = "UPDATE visitors SET affiliate_code = '" . escape($liveform->get_field_value('special_offer_code')) . "' WHERE id = '" . $_SESSION['software']['visitor_id'] . "'";
                    $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
                }
                
                // prepare to update affiliate code for order
                $sql_affiliate_code = ", affiliate_code = '" . escape($liveform->get_field_value('special_offer_code')) . "'";
            }
        }
    }
    ...
    $query =
        "UPDATE orders
        SET
            special_offer_code = '" . escape($liveform->get_field_value('special_offer_code')) . "'
            $sql_affiliate_code
...
// else the visitor clicked the checkout button
} else {
    ...
    // if there is an error or notice
    // send user back to shopping cart page
    if (
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            */
            ...
            foreach ($ship_tos as $ship_to) {
                ...
                // assume that the product preparation time is 0 until we find valid products with higher preparation times
                ...
                foreach ($order_items as $order_item) {
                    ...
                    if (validate_product_for_destination($order_item['product_id'], $country_code, $state_code) == true) {
                        ...
                        // if product has the greatest preparation time so far, update product preparation time
                        if ($order_item['preparation_time'] > $product_preparation_time) {
                            $product_preparation_time = $order_item['preparation_time'];
                        }
                    } else {
                    ...
                    if ($shipping_method_is_valid == true) {
                        ...
                        if ($arrival_date != '0000-00-00') {
                            ...
                            // else there is not a shipping cut-off, so determine if this shipping method is valid by looking at the transit time
                                ...
                                // if current time is before end of day time, then order start date is today
                                if ($current_time < ECOMMERCE_END_OF_DAY_TIME) {
                                    $order_start_date = date('Y-m-d', $current_timestamp);
                                    
                                // else current time is after end of day time, so order start date is tomorrow
                                } else {
                                    $order_start_date = date('Y-m-d', $current_timestamp + 86400);
                                }
                                
                                // get shipping start date by adding product preparation time to order start date
                                $shipping_start_date = date('Y-m-d', strtotime($order_start_date) + (86400 * $product_preparation_time));
                                ...
                                // add base transit days for shipping method to total shipping days
                                $total_shipping_days = $base_transit_days;

                                // if adjust transit is on for this shipping method, get transit adjustment days for country
                                if ($adjust_transit == 1) {
                                    $query = "SELECT transit_adjustment_days FROM countries WHERE code = '" . escape($country_code) . "'";
                                    $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
                                    $row = mysqli_fetch_assoc($result);

                                    // add transit adjustment days for country to total shipping days
                                    $total_shipping_days += $row['transit_adjustment_days'];
                                }
                                
                                // get excluded transit dates for this shipping method
                                $query = "SELECT date FROM excluded_transit_dates WHERE shipping_method_id = '$shipping_method_id'";
                                $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
                                
                                $excluded_transit_dates = array();
                                
                                while ($row = mysqli_fetch_assoc($result)) {
                                    $excluded_transit_dates[] = $row['date'];
                                }
                                
                                $date = '';
                                $count = 0;
                                
                                // calculate the earliest arrival date by looping through days and determining if days are excluded
                                while ($count <= $total_shipping_days) {
                                    // if date is blank, then set date to shipping start date
                                    if ($date == '') {
                                        $date = $shipping_start_date;
                                    ...
                                    if (
                                        (($day_name != 'Sunday') || ($transit_on_sunday == 1))
                                        && (($day_name != 'Saturday') || ($transit_on_saturday == 1))
                                        && (in_array($date, $excluded_transit_dates) == false)
                                    ) {
                                        $count++;
                                    }
                                }
                                
                                $earliest_arrival_date = $date;
                                
                                // if earliest arrival date is less than or equal to requested arrival date, then the shipment can be sent to the recipient in time for the requested arrival date
                                if ($earliest_arrival_date <= $arrival_date) {
                                    $shipment_can_be_delivered_in_time = true;
                                } else {
                                    $shipment_can_be_delivered_in_time = false;
                                }
            ...
            if (mysqli_num_rows($result) > 0) {
                $row = mysqli_fetch_assoc($result);
                header('Location: ' . URL_SCHEME . $_SERVER['HTTP_HOST'] . PATH . get_page_name($_POST['next_page_id_with_shipping']) . '?ship_to_id=' . $row['id']);
            ...
            // else all ship tos are complete, so send user to billing information or order preview screen   
                ...
                if ($billing_complete == 0) {
                    header('Location: ' . URL_SCHEME . $_SERVER['HTTP_HOST'] . PATH . get_page_name($_POST['next_page_id_without_shipping']));
