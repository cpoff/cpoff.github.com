...
if ((isset($_GET['page_id'])) && ($_GET['page_id'] != '')) {
    ...
    if (check_edit_access($folder_id) == false) {
        log_activity('access denied to view fields for ' . $form_type . ' because user does not have access to modify folder that ' . $form_type . ' is in', $_SESSION['sessionusername']);
    ...
    $output_form_designer_content_heading = 'Edit ' . ucwords($form_type);
    $output_form_designer_content_subheading = 'Add fields to this ' . $form_type . '.';
    $delete_data_warning = '';
...
$query = "SELECT
         ...
         WHERE form_fields." . $form_type_identifier_id . " = '" . escape($_GET[$form_type_identifier_id]) . "'
...
echo
    ...
    <div id="button_bar">
        <a href="add_field.php?' . $form_type_identifier_id . '=' . h(urlencode($_GET[$form_type_identifier_id])) . '&send_to=' . h(urlencode($_GET['send_to'])) . '" >Create Form Field</a>
        ' . $output_layout_buttons . '
    </div>
    <div id="content">
