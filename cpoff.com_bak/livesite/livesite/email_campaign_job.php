...
foreach ($email_recipients as $email_recipient) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        // If this is not a gift card campaign and this contact is not the manually entered email
        ...
        // the original contact might have been deleted or merged.  Gift card emails are considered
        // transactional (not marketing), so we always want to send to recipient, regardless of
        // whether recipient is opted-out or not.  We consider all other types of campaigns,
        // including auto-campaigns as marketing campaigns (non-transactional), so we don't want to
        // send to recipient in that case if he/she opted-out.

        if (
            ($email_recipient['action'] != 'gift_card_ordered')
        ...
        if ($email_recipient['format'] == 'plain_text') {
            // If this is not a gift card email, then continue to prepare organization
            // and email preferences info.  We don't need to output organization info
            // for a single gift card email.  Also, we don't include the email preferences
            // link for gift card emails because it does not work because there is no contact
            // for recipient.
            if ($email_recipient['action'] != 'gift_card_ordered') {
                $organization = '';
