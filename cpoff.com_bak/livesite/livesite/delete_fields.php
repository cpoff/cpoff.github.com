...
if ((isset($_POST['page_id'])) && ($_POST['page_id'] != '')) {
    ...
    if (check_edit_access($folder_id) == false) {
        log_activity('access denied to delete fields from ' . $form_type . ' because user does not have access to modify folder that ' . $form_type . ' is in', $_SESSION['sessionusername']);
...
if ($_POST['fields']) {
    ...
    if ($form_type == 'product form') {
        ...
        if ($number_of_fields > 0) {
            ...
            log_activity("$number_of_fields field(s) were deleted from $form_type ($form_name)", $_SESSION['sessionusername']);
