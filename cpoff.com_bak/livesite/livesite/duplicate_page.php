...
if (($user['role'] == '3') && ($user['create_pages'] == FALSE)) {
    log_activity("access denied because user does not have access to duplicate pages", $_SESSION['sessionusername']);
    output_error('Access denied. <a href="javascript:history.go(-1)">Go back</a>.');
}

// get original page's info
$result = mysqli_query(db::$con, "SELECT * FROM page WHERE page_id = '" . escape($_GET['id']) . "'") or output_error('Query failed');
...
if (
    ...
    (
        ...
        || ($page_type == 'change random password')
...
update_tag_cloud_keywords_for_page($new_page_id, $page_search, $page_meta_keywords, 0, '');

// get region info
$result=mysqli_query(db::$con, "SELECT pregion_content, pregion_order, collection FROM pregion WHERE pregion_page = '" . escape($_GET['id']) . "' ORDER BY pregion_order") or die ('Query failed');
// create duplicate regions
while($row=mysqli_fetch_array($result)) {
    $region_name = time() .'_'. $row[pregion_order];
    $result_insert=mysqli_query(db::$con, "INSERT INTO pregion (pregion_name, pregion_content, pregion_page, pregion_order, collection, pregion_user, pregion_timestamp) VALUES ('" . escape($region_name) . "', '" . escape($row['pregion_content']) . "', '" . escape($new_page_id) . "', '" . escape($row['pregion_order']) . "', '" . escape($row['collection']) . "', '" . escape($user['id']) . "', UNIX_TIMESTAMP())") or output_error('Query failed');
}
...
if (
    ...
    $query = "SELECT * FROM form_fields WHERE page_id = '" . escape($_GET['id']) . "'";
    $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
...
if ($page_type == 'form list view') {
    $query = "SELECT * FROM form_list_view_filters WHERE page_id = '" . escape($_GET['id']) . "'";
    $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
    ...
    // Get browse fields.
    $query = "SELECT * FROM form_list_view_browse_fields WHERE page_id = '" . escape($_GET['id']) . "'";
    $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
...
if ($page_type == 'form view directory') {
    $query =
        ...
        WHERE form_view_directory_page_id = '" . escape($_GET['id']) . "'";
    $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
...
if ($page_type == 'calendar view') {
    $query = "SELECT calendar_id FROM calendar_views_calendars_xref WHERE page_id = '" . escape($_GET['id']) . "'";
    $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
...
if ($page_type == 'calendar event view') {
    $query = "SELECT calendar_id FROM calendar_event_views_calendars_xref WHERE page_id = '" . escape($_GET['id']) . "'";
    $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
...
if ($page_type == 'search results') {
    ...
    $query = "SELECT search_catalog_items, product_group_id FROM search_results_pages WHERE page_id = '" . escape($_GET['id']) . "'";
    $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
    ...
    update_tag_cloud_keywords_for_search_results_page_type($new_page_id, $new_search_catalog_items, $new_product_group_id);
}
...
if (check_for_page_type_properties($page_type) == true) {
    ...
    $properties = get_page_type_properties($_GET['id'], $page_type);
    ...
    $properties['page_id'] = $new_page_id;
    ...
    if ($page_type == 'form list view' or $page_type == 'form item view') {
        ...
        $properties = get_page_type_properties($_GET['id'], $page_type, 'b');
...
$query = "SELECT * FROM comments WHERE page_id = '" . escape($_GET['id']) . "'";
$result = mysqli_query(db::$con, $query) or output_error('Query failed.');
...
if (
    ...
    file_put_contents(LAYOUT_DIRECTORY_PATH . '/' . $new_page_id . '.php', file_get_contents(LAYOUT_DIRECTORY_PATH . '/' . $page_id . '.php'));
}

log_activity("page ($original_page_name) was duplicated", $_SESSION['sessionusername']);
...
header('Location: ' . URL_SCHEME . $_SERVER['HTTP_HOST'] . PATH . SOFTWARE_DIRECTORY . '/edit_page.php?id=' . $new_page_id . '&from=' . $_GET['from']);
...
function get_duplicate_form_name_number($form_name, $number = 1)
{
    ...
    $query = "SELECT form_name FROM custom_form_pages where form_name = '" . escape($form_name) . "[" . $number . "]'";
    $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
