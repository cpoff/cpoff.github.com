...
function get_shopping_cart($properties) {
    ...
    // if an order preview page has not been saved in the session yet,
    // get order preview page for this order pathway (in case shipping and billing information screens can be bypassed)
    if (isset($_SESSION['ecommerce']['order_preview_page_id']) == false) {
        $query = "SELECT next_page_id FROM billing_information_pages WHERE page_id = '" . escape($next_page_id_without_shipping) . "'";
        $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
        $row = mysqli_fetch_assoc($result);
        $_SESSION['ecommerce']['order_preview_page_id'] = $row['next_page_id'];
    }
        ...
        if ($ship_tos) {
            ...
            $output_shopping_cart =
                ...
                <form action="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/shopping_cart.php" method="post" style="margin: 0px 0px 15px 0px">
                    ' . get_token_field() . '
                    <input type="hidden" name="page_id" value="' . $page_id . '" />
                    <input type="hidden" name="next_page_id_with_shipping" value="' . $next_page_id_with_shipping . '" />
                    <input type="hidden" name="next_page_id_without_shipping" value="' . $next_page_id_without_shipping . '" />
                    <input type="hidden" name="require_cookies" value="true" />
