...
if ($page_id != 0) {
    ...
    switch ($page_type) {
        ...
        case 'billing information':
            $form_type = 'custom billing form';
            break;
            
        case 'shipping address and arrival':
            $form_type = 'custom shipping form';
            break;
    }
    ...
    if (check_edit_access($folder_id) == false) {
        log_activity('access denied to edit field on ' . $form_type . ' because user does not have access to modify folder that ' . $form_type . ' is in', $_SESSION['sessionusername']);
    ...
    $output_form_designer_subnav_subheading = ucwords($form_type) . ': ' . h($form_name);
    $output_form_designer_content_heading = 'Edit ' . ucwords($form_type) . ' Field';
    $output_form_designer_content_subheading = 'View or update this ' . $form_type . ' field.';
...
if (!$_POST) {
    ...
    $query = "SELECT id
             FROM form_fields
             WHERE (" . $form_type_identifier_id . " = '" . escape(${$form_type_identifier_id}) . "') AND (sort_order < $sort_order)
            ...
            <form action="edit_field.php" method="post">
                ...
                <table class="field">
                    ...
                    <tr id="position_row" style="' . $position_row_style . '">
                        <td>Position:</td>
                        <td><select name="position">' . select_field_position($position, $_GET['id'], ${$form_type_identifier_id}, $form_type) . '</td>
                    </tr>
    ...
    if ($_POST['submit_delete'] == 'Delete') {
        ...
        log_activity($form_type . ' field (' . $_POST['name'] . ') on ' . $form_type . ' (' . $form_name . ') was deleted', $_SESSION['sessionusername']);
        
        $liveform_view_fields->add_notice('The field has been deleted.');

        // forward user to view fields screen
        header('Location: ' . URL_SCHEME . HOSTNAME . PATH . SOFTWARE_DIRECTORY . '/view_fields.php?' . $form_type_identifier_id . '=' . $_POST[$form_type_identifier_id] . '&send_to=' . urlencode($_POST['send_to']));
    ...
    // else field was not selected for deletion
                ...
                if (
                    ...
                    if ($target_field_name != '') {
                        $target_field_id = db_value(
                            ...
                            FROM form_fields
                            WHERE
                                ($form_type_identifier_id = '" . escape($_POST[$form_type_identifier_id]) . "')
                                AND (name = '" . escape($target_field_name) . "')
                                AND (type = 'pick list')");
        ...
        $query = "SELECT id
                 FROM form_fields
                 WHERE (" . $form_type_identifier_id . " = '" . escape(${$form_type_identifier_id}) . "') AND (id != '" . escape($_POST['id'])  . "')
                 ORDER BY sort_order";
        $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
        ...
        log_activity($form_type . ' field (' . $_POST['name'] . ') on ' . $form_type . ' (' . $form_name . ') was modified', $_SESSION['sessionusername']);
