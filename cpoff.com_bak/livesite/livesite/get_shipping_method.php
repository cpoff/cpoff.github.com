    ...
    if ($layout_type == 'system') {
        ...
        if ($products_are_valid == true) {
            ...
            if ($arrival_date > '0000-00-00') {
                ...
                $query = "SELECT MAX(products.preparation_time)
                         FROM order_items
                         LEFT JOIN products on products.id = order_items.product_id
                         WHERE ship_to_id = '" . escape($_GET['ship_to_id']) . "'";
                $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
                $row = mysqli_fetch_row($result);
                $product_preparation_time = $row[0];
                ...
                foreach ($shipping_methods as $key => $shipping_method_id) {
                    ...
                    // else there is not a shipping cut-off, so determine if this shipping method is valid by looking at the transit time
                        ...
                        // if earliest arrival date is greater than requested arrival date, then this shipping method is not valid, so remove it from shipping methods array
            ...
            if ($requested_arrival_date > '0000-00-00') {
                ...
                foreach ($shipping_methods as $key => $shipping_method_id) {
                    ...
                    $query =
                        ...
                        LEFT JOIN arrival_dates ON shipping_cutoffs.arrival_date_id = arrival_dates.id
                        WHERE
                            (arrival_dates.arrival_date = '" . escape($requested_arrival_date) . "')
                            AND (shipping_cutoffs.shipping_method_id = '$shipping_method_id')";
                    $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
                    ...
                    // else there is not a shipping cut-off, so determine if this shipping method is valid by looking at the transit time
                        ...
                        // if earliest arrival date is greater than requested arrival date, then this shipping method is not valid, so remove it from shipping methods array
                        if ($earliest_arrival_date > $requested_arrival_date) {
                            unset($shipping_methods[$key]);
                        }
                    }
                }
