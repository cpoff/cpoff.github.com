...
if ($form == 1) {
    submit_custom_form(array('page_type' => 'shipping address and arrival'));
}
...
$query = "SELECT states.id
         ...
         WHERE countries.code = '" . escape($liveform->get_field_value('country')) . "'
...
if (isset($_POST['arrival_date'])) {
    ...
    if (($liveform->check_form_errors() == false) && ($arrival_date != '0000-00-00')) {
        ...
        // loop through all products in cart for this recipient, to get greatest preparation time
        foreach ($order_items as $key => $value) {
            // if product is valid for recipient's address
            if (validate_product_for_destination($order_items[$key]['product_id'], $liveform->get_field_value('country'), $liveform->get_field_value('state')) == true) {
                // if product has the greatest preparation time so far, update product preparation time
                if ($order_items[$key]['preparation_time'] > $product_preparation_time) {
                    $product_preparation_time = $order_items[$key]['preparation_time'];
                }
            }
        }
        ...
        if (count($shipping_methods) == 0) {
            ...
            */
            ...
            foreach ($order_items as $key => $value) {
                /* begin: get valid zone for order item */
                ...
                // loop through all valid zones for order item
                foreach ($zones_for_order_item as $zone_id) {
                ...
                $query = "SELECT
                         ...
                         WHERE
                            (shipping_methods_zones_xref.zone_id = '$zone_id')
        ...
        foreach ($shipping_methods as $shipping_method_id) {
            ...
            // else there is not a shipping cut-off, so determine if this shipping method is valid by looking at the transit time
                ...
                if ($adjust_transit == 1) {
                    $query = "SELECT transit_adjustment_days FROM countries WHERE code = '" . escape($liveform->get_field_value('country')) . "'";
                    $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
                    $row = mysqli_fetch_assoc($result);
                ...
                // if earliest arrival date is less than or equal to requested arrival date, then there is at least one valid shipping method for requested arrival date
...
$address_verified = verify_us_address($liveform, 'shipping', $address_verified);
...
if (isset($_SESSION['sessionusername'])) {
    ...
    // if user id was found, add ship to data to address book
    if ($user_id) {
        // find recipient in address book
        $query = "SELECT id FROM address_book WHERE user = '$user_id' AND ship_to_name = '" . escape($ship_to_name) . "'";
        $result = mysqli_query(db::$con, $query) or output_error('Query failed.');
        $row = mysqli_fetch_assoc($result);
